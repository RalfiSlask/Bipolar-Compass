...
import { IConversationLog } from '@/app/types/ai';
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    let conversation = await AiCollection.findOne({
    ...
    if (!conversation) {
      ...
      const result = await AiCollection.insertOne(newConversation);
      conversation = {
        ...newConversation,
    ...
    const botData = await botCollection.findOne({
